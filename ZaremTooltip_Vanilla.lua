...
do
    ...
    local texture = moverFrame:CreateTexture("ARTWORK")
...
GameTooltip:HookScript("OnTooltipSetUnit", function(self)
    ...
    if UnitIsPlayer(unit) then
        ...
        if config.classHealth then
            GameTooltipStatusBar:SetStatusBarColor(cr, cg, cb)
        end
        ...
        if guild then
            ...
            for i = 3, self:NumLines() do
                local guildLine = _G["GameTooltipTextLeft" .. i]

                if strfind(guildLine:GetText(), guild) then
                    if config.yourGuild and UnitIsInMyGuild(unit) and UnitIsSameServer(unit, "player") and UnitIsFriend(unit, "player") then
                        guildLine:SetTextColor(config.yourGuildColor[1], config.yourGuildColor[2], config.yourGuildColor[3])
                    else
                        if config.reactionGuild then
                            guildLine:SetTextColor(r, g, b)
                        else
                            guildLine:SetTextColor(config.guildColor[1], config.guildColor[2], config.guildColor[3])
                        end
                    end

                    if config.guildRank then
                        guildLine:SetFormattedText("<%s> %s%s", guild, config.gRankColor, rank)
                    else
        ...
        GameTooltipStatusBar:SetStatusBarColor(config.status[1], config.status[2], config.status[3])
...
GameTooltipStatusBar:HookScript("OnValueChanged", function(self, value)
    ...
    if config.classHealth and UnitIsPlayer(unit) then
        ...
        self:SetStatusBarColor(config.status[1], config.status[2], config.status[3])
    end
end)
