    ...
    if UnitIsBattlePetCompanion(unit) then
        ...
        for i = 2, tooltip:NumLines() do
            ...
            if lines and lines:GetText() then
                if strfind(lines:GetText(), UNIT_LEVEL_TEMPLATE) then
                    i = i - 1
        ...
        if lines and lines:GetText() then
            ...
            if i > 2 then
                ...
                if config.hideFaction then
                    if lines:GetText() == FACTION_HORDE or lines:GetText() == FACTION_ALLIANCE then
                        lines:SetText(nil)
                    end
                end
            end
